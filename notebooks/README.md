...
| 13 | [`13_pauli_propagation_qbm.ipynb`](13_pauli_propagation_qbm.ipynb) | **Pauli propagation**: the thermal state as a sparse Pauli sum evolved under imaginary time; exact for commuting Hamiltonians, topology-agnostic; the locally normalised sampler with exact likelihoods; training a QBM for generative modelling in the Pauli basis; and **how coefficient- vs Pauli-weight-truncation levels trade cost for accuracy across a training run** (the efficiency frontier). |
